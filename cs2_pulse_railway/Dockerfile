...
3. **Variáveis de ambiente** (separador Variables do serviço):
   ...
   DATA_DIR = /data
   ```
   Opcional — só se quiseres um limite diferente do volume:
   ```
   MAX_DB_BYTES = 400000000
   ```
   (o valor é em bytes; 400000000 ≈ 400MB. A limpeza automática dispara
   acima disto e desce até 80% desse valor, nunca apagando as 20 análises
   mais recentes. Ajusta se mudares o tamanho do volume.)
